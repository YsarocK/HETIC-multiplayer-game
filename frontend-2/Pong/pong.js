...
socket.on("switchPlayerIndex", (players) => {
  ...
  document.querySelector("#playerNumber").innerHTML =
    players.findIndex((player) => player.id === id) + 1;

  player = players.findIndex((player) => player.id === id) + 1;
});
    ...
    ball = new Ball(
      {
        ...
        onEscape: (result) => {
          console.log("onEscape", result, player);
          if (player == 1) {
            console.log("socket emit player 1 endgame");
            socket.emit(
              "game_end",
              "Gagnant: " + (result.winner === "left" ? "Gauche" : "Droit")
            );
          }
        },
      },
